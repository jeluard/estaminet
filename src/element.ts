...
export abstract class BaseProviderElement extends HTMLElement {
  ...
  fireApiFailedEvent(error: Error) {
    this.dispatchEvent(
      new CustomEvent('api-failed', {
        bubbles: true,
        composed: true,
        detail: { error },
      })
  ...
  attributeChangedCallback(name: string, oldValue: any, newValue: any) {
    console.log("attributeChangedCallback", name, newValue, this.provider)
    this.dispatchEvent(
          ...
          if (provider) {
            ...
            ApiPromise.create({ provider: wsProvider })
              ...
              .catch(() => {
                ...
                this.fireApiFailedEvent(new Error('Failed to create API'));
              });
          } else {
            this.api = undefined;

            this.fireApiFailedEvent(new Error('No matching provider'));
          }
        } else {
          this.api = undefined;
